...
def process_model(model_id):
    ...
    toka, _, token_infos = load_analyzers(model_id, indicator_ix=indicator_ix, avoid_loading_model=True, trust_remote_code=True)
    try:
...
def format_info(raw_info, target='notebook', num_tokens=10):
    ...
    if target in ['markdown', 'notebook']:
        ...
        info['Examples'] = ', '.join(escape_token_for_markdown(t['readable_vocab']) for t in verified_cand_nosb[:num_tokens])
    if target == 'markdown':
        info['Examples'] = ', '.join(escape_token_for_markdown(t['readable_vocab']) for t in verified_cand_nosb[:num_tokens])
